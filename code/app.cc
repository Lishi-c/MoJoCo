...
void controller(const mjModel* m, mjData* data) {
    ...
    // 3. Auto-Goal (作业代码)
    if (sim->run) {
        double car_x = data->qpos[0];
        double car_y = data->qpos[1];
        double ball_x = data->mocap_pos[0];
        double ball_y = data->mocap_pos[1];

        double dist = std::sqrt(std::pow(car_x - ball_x, 2) + std::pow(car_y - ball_y, 2));

        if (dist < 0.5) {
            double new_x = ((rand() % 500) / 100.0) - 2.5;
            double new_y = ((rand() % 500) / 100.0) - 2.5;
            data->mocap_pos[0] = new_x;
            data->mocap_pos[1] = new_y;
            if (data->mocap_pos[2] < 0.1) data->mocap_pos[2] = 0.2;
        }
    }
}
