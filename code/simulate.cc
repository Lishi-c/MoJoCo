...
void Simulate::Render() {
  ...
  // ==================== [added code] ====================
  if (this->d && this->m) {
      // 1. acquire real-world physical data
      double vx = this->d->qvel[0];
      double vy = this->d->qvel[1];
      double real_speed_ms = std::sqrt(vx * vx + vy * vy);
      double speed_kmh = real_speed_ms * 3.6;

      // print data to control platform
      static int print_counter = 0;
      if (print_counter++ % 60 == 0) {
          std::printf(">> [Data Log] Time: %.2f | Pos: (%.2f, %.2f) | Speed: %.2f km/h | Acc: %.2f\n",
              this->d->time,
              this->d->qpos[0], this->d->qpos[1],
              speed_kmh,
              this->d->qacc[0]);
      }

      // 2. clamp the gauge's maximum value
      double display_speed = speed_kmh;
      if (display_speed > 10.0) display_speed = 10.0;

      // 3. plot a 3D dashboard
      float x = (float)this->d->qpos[0];
      float y = (float)this->d->qpos[1];
      float z = (float)this->d->qpos[2];
      g_dashboard.renderFollow((float)display_speed, x, y, z);

      // 4. localize the scale markings
      char info[1024];
      std::snprintf(info, 1024,
          "DASHBOARD SYSTEM\n\n"
          "State:\n"
          "  Time:   %.1f s\n\n"
          "Dynamics:\n"
          "  Speed:  %.2f km/h\n" // display ture velocity
          "  Gauge:  0 - 10 km/h\n\n"
          "Status:   PHYSICS REAL",
          this->d->time, speed_kmh);

      mjrRect text_rect = { 20, 0, 300, rect.height };
      mjr_overlay(mjFONT_NORMAL, mjGRID_TOPLEFT, text_rect, info, nullptr, &this->platform_ui->mjr_context());
  }
  // ===================================================================
